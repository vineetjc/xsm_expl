...
void GInstall(struct Gsymbol **headref, char *name, struct Typetable *type, int size, struct Paramstruct *paramlist); // Creates a global symbol table entry.
...
void GAssignType(struct Gsymbol **headref, struct Typetable *type); // Assigns the types to all the remaining variables in a global declaration.

struct Paramstruct* createParamNode(char *varname, struct Typetable *type); // Creates a parameter in the parameter list.
...
void TypeTableCreate(struct Typetable **headref); // initialising the type table with primitive datatypes

struct Typetable* TLookup(struct Typetable **headref, char *name); // Search through the type table and return pointer to type table entry of type 'name'. Returns NULL if entry is not found.

struct Typetable* createTypeNode(char *name, int size, struct Fieldlist *fields); // creates and returns a type table node

void TInstall(struct Typetable **headref, char *name, int size, struct Fieldlist *fields); // creates a type table entry in the type table

struct Fieldlist* FLookup(struct Typetable **typeref, char *name); // Searches for a field of given 'name' in the 'fieldlist' of the given user-defined type and returns a pointer to the field entry

int getSize(struct Typetable **typeref); // Returns the amount of memory words required to store a variable of the given type

void addFields(struct Typetable **typeref, struct Fieldlist *fields); // adds fields to the given type entry

struct Typetable* getFieldType(struct tnode **headref); // gets the type of the field 

void xos_heapcall(char *func, int arg1, int p); // codeGen for Alloc, Free and Initialise
